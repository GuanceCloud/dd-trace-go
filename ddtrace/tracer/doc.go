...
//
// Guance OTLP span metrics use a separate concentrator and keep native trace
// payloads on 0.4, with Datadog-Client-Computed-Stats: yes, so the Datadog Agent
// does not recompute stats already exported through OTLP. This override also
// applies when DD_TRACE_AGENT_PROTOCOL_VERSION requests 1.0.
//
